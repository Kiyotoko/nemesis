...
import org.nemesis.grpc.game.ChangeResponse;
import org.nemesis.grpc.game.NemesisGrpc.NemesisImplBase;
...
public class NemesisServer {
	...
	public class NemesisDispatchHelper implements DispatchHelper<Corresponding<?>, StatusResponse> {
		...
		private final Set<Identifiable> destroyed = new HashSet<>();
		...
		public StatusResponse associated() {
			return StatusResponse.newBuilder()
					...
					.addAllDestroyed((Iterable<com.karlz.grpc.exchange.Identifiable>) (Iterable<?>) Corresponding
							.transform(destroyed))
					.build();
		}
		...
		public Set<Identifiable> getDestroyed() {
			return destroyed;
		}
	}
